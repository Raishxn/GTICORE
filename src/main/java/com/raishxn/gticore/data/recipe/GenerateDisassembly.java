...
import net.minecraft.core.registries.BuiltInRegistries; // <--- IMPORTAÇÃO NECESSÁRIA
...
import static com.raishxn.gticore.common.data.GTIRecipeTypes.DISASSEMBLY_RECIPES;
import static com.raishxn.gticore.common.data.GTIRecipes.DISASSEMBLY_RECORD;
...
public class GenerateDisassembly {
    ...
    public static void generateDisassembly(GTRecipeBuilder r, Consumer<FinishedRecipe> p) {
        ...
        if (c == null) {
            GTICORE.LOGGER.atError().log("Recipe {} without output", r.id);
            return;
        }
            ...
            if (itemList != null) {
                itemList.forEach(content -> {
                    ...
                    if (content.chance == ChanceLogic.getMaxChancedValue() && !item.isEmpty() && !item.hasTag()) {
                        ...
                        if (cal || !isExcludeItems(itemId, inputItem)) {
